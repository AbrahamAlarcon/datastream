...
java -Dapp.name=${APP_NAME} -Dweatherkey=${WEATHER_KEY} -Dspringprofilesactive=${ENV} ${PEERS} -Xmx${JAVA_HEAP_XMX} -Xms${JAVA_HEAP_XMS} -XX:+UseConcMarkSweepGC -XX:+CMSParallelRemarkEnabled -XX:+UseCMSInitiatingOccupancyOnly -XX:CMSInitiatingOccupancyFraction=${CMS_INIT_OCCUPANCY_FRACTION} -XX:+ScavengeBeforeFullGC -XX:+CMSScavengeBeforeRemark -Dsun.net.inetaddr.ttl=${INETADDR_TTL} -Dserverport=${SERVER_PORT} -Xrunjdwp:server=y,transport=dt_socket,address=${DEBUG_PORT},suspend=n -jar ${APPLICATION_EXECUTABLE}